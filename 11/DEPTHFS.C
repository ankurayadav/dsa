...
{	struct node *arr[MAX]  ;
	...
	int i ;
	clrscr();
	//inserting nodes at the vertex v1
	v1=getnode_write(2) ;
	arr[0]=v1 ;
	v1->next = v2= getnode_write(3)  ;
	v2->next = NULL ;

	//inserting nodes at the vertex v2
	v1=getnode_write(1) ;
	arr[1] = v1 ;
	...
	//inserting nodes at the vertex v3
	v1=getnode_write(1) ;
	arr[2] = v1;
	...
	printf("the list of nodes visited in this traversal are :\n\n\n\t");
	dfs(1,arr)  ;
